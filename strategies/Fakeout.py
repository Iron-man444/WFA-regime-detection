...
STRATEGY_PARAMS = {
    ...
    "limit_dist_pct": {"default": 0.05, "start": 0.01, "step": 0.02, "stop": 0.1},
...
def execute_strategy(ohlcv: pd.DataFrame, params: dict) -> dict:
    ...
    limit_dist_pct = float(params.get("limit_dist_pct", 0.05))
    ...
    # ==========================================
    # 2. TEPE/DİP VE LİMİT EMİR SEVİYELERİ
    # ==========================================
    highest_high = high.rolling(window=bars_n).max()
    lowest_low = low.rolling(window=bars_n).min()

    # MQL5'teki (1 + LimitDistPct / 100.0) mantığı
    sell_limit_price = highest_high * (1 + (limit_dist_pct / 100.0))
    buy_limit_price = lowest_low * (1 - (limit_dist_pct / 100.0))
    ...
    # ==========================================
    # LONG: RSI dipteyse VE mumun en düşük fiyatı "Buy Limit" seviyemize çarpmış/altına inmişse
    long_entries = (rsi <= rsi_os) & (low <= buy_limit_price) & time_filter

    # SHORT: RSI şişkinse VE mumun en yüksek fiyatı "Sell Limit" seviyemize çarpmış/üstüne çıkmışsa
    short_entries = (rsi >= rsi_ob) & (high >= sell_limit_price) & time_filter

    # ==========================================
    # 5. DİNAMİK ÇIKIŞ SİNYALLERİ (TP/SL YERİNE)
    # ==========================================
    ...
    # Aynı mumda çakışan sinyalleri temizle
